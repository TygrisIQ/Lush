...
class TrackListViewModel : ViewModel() {
    var musicList = mutableListOf<Track>()
    ...
    fun getMusicList(context: Context){
        ...
        query?.use { cursor ->
            ...
            while (cursor.moveToNext()){
                ...
                val thumbnail = getThumbnail(context, contentUri)
                musicList += Track(
                    ...
                    getThumbnail(context,contentUri,index)
    ...
    private fun getThumbnail(context: Context, uri: Uri,index : Int): Bitmap{
        val mediaMetadataRetriever = MediaMetadataRetriever()
        mediaMetadataRetriever.setDataSource(context,uri)
        val byte1: ByteArray? = mediaMetadataRetriever.getEmbeddedPicture()
        mediaMetadataRetriever.release()
        return if(byte1 != null){
